...
- **박싱/언박싱 방식은 시간이 아주 오래 걸리는 방식이므로 가능하면 사용하지 말고 제네릭을 사용해야 한다!!!!!⭐⭐⭐**
    - 박싱 과정에서 아예 새로운 개체를 만들기 떄문에 이로 인해 시간이 박싱을 사용하지 않을 떄보다 20배 많이 걸린다.
    - 언박싱 과정에서는 박싱 과정보다 4배가 더 걸린다.
    - 힙에서는 스택보다 상대적으로 많이 느리다.
- 근데 그럼 왜 사용하는가?
    - ArrayList 처럼 복합형으로 타입에 한정되지 않는 object 타입일 떄 많이 쓴다.
    - 후에 부작용 때문에 제너릭 컬렉션을 닷넷에서 제공
