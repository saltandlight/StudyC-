...
- CLR은 어셈블리의 메타데이터 사용 -> 이 타이븓ㄹ에 대한 정보 추출 -> 타입에 대한 정보를 서술하는 자료구조를 생성하게 될 것
- 이 스레드에서는 M3 메서드 실행 전에 코드 실행했으므로, Int32와 String 타입과 같이 공통적으로 사용될 확률이 높은 타입에 대한 타입 객체에 대한 정보가 이미 만들어져 있음
- 그러나 여기서는 생략하겠음

![](pic6.PNG)
- 힙 위에 만들어지는 모든 객체들은 타입 객체 포인터와 동기화 블록 인덱스라는 두 개의 추가 멤버를 포함하고 있음
- 타입 선언 시, 정적 데이터 필드 포함 가능
- 이런 정적 데이터 필드의 할당에 소요되는 메모리는 타입 객체를 만드는 시점에서 할당됨
- 각 타입 객체 안에는 타입 내의 메서드 하나당 한 개의 항목이 포함된 메서드 테이블도 만들어짐
- CLR이 메서드의 실행에 필요한 모든 타입 객체 만들었고, 실행하려는 코드 컴파일도 완료 -> 컴파일된 네이티브 코드의 실행 허용할 것
- M3 메서드의 프롤로그 코드 실행 시, 스레드의 스택상에 지역 변수를 위한 메모리를 밑의 그림처럼 할당하게 됨
- 프롤로그 코드의 일부로 CLR은 모든 지역 변수를 null 또는 숫자 0으로 초기화를 자동으로 처리함
- 그러나 C# 컴파일러는 소스 코드상에서 명시적으로 초기값을 할당하지 않을 경우 컴파일러 차원에서 할당되지 않은 지역변수를 사용하려고 한다는 오류 메시지를 나타낼 것

![](pic7.PNG)
- 그 다음, M3 메서드의 코드를 실행하면서 Manager 객체가 만들어지게 됨 -> 관리되는 힙에 Manager 타입의 객체를 만들게 됨
- Manager 객체 역시 타입 객체 포인터와 동기화 블록 인덱스를 가지게 됨
- 이 객체는 또한 Manager 타입에서 정의한 모든 인스턴스 데이터 필드의 내용들을 저장하는데 필요한 메모리 공간도 포함하고 있음
- 힙에 새로운 객체가 생성될 때, CLR은 자동으로 내부의 타입 객체 포인터가 객체의 정확한 타입을 가리키도록 초기화됨
- 이 경우, Manager 타입 객체가 그 대상이 됨
- CLR은 동기화 블록 인덱스와 모든 개체의 인스턴스 필드들을 NULL 또는 0으로 초기화한 후 다음 타입의 생성자 호출 -> 인스턴스의 데이터 필드를 수정하게 됨
- 이후 new 연산자는 만들어진 Manager 객체의 메모리 주소를 반환하여 스레드의 스택상에 할당된 변수 e에 저장하게 됨

- 다음 명령문으로 이동하면 Employee 타입의 Lookup 정적 메서드를 호출하게 됨
- 이 정적 메서드 호출 시 JIT 컴파일러는 호출하려는 정적 메서드를 정의하는 정확한 타입에 대한 타입 객체를 찾게 됨
- JIT 컴파일러는 타입 객체의 메서드 테이블 안에서 호출하려는 메서드에 대한 항목을 찾아, 필요한 경우 해당 메서드에 대한 JIT 컴파일 수행,
 JIT 컴파일이 완료된 코드를 실행함
- ... 생략



- CLR이 타입 객체를 생성 시 CLR은 이들에 대한 멤버도 초기화함
- CLR이 프로세스 안에서 실행될 떄 내부적으로 System.Type이라는 MSCORLIB.DLL 안에 선언되어 있는 타이벵 대해서 초기화를 즉각적으로 수행함!!
- Employee와 Manager 타입 객체는 이 타입에 대한 "인스턴스", 이 타입 객체의 포인터 멤버는 System.Type 타입 객체를 가리키도록 초기화됨

- System.Type 객체는 그 자체로 객체 -> 내부적으로 타입 객체 포인터가 멤버로 들어있음
- 이 멤버는 자기 자신을 가리킴
- System.Type 타입 객체가 그 자체로 객체의 타입 객체이므로 그럼

- System.Object 타입의 GetType 메서드는 단순히 객체의 타입 객체 포인터 멤버에 지정된 주소를 반환함
- 타입 객체를 포함해서 시스템에서 사용 가능한 모든 객체들의 정확한 타입을 알아낼 수 있음
